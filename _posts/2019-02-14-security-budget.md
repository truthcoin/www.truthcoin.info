...
Bitcoin's ["security budget"](https://medium.com/coinmonks/bitcoin-security-in-one-chart-694ee3ed8c2d) is the total amount of money we pay to miners (or, if you prefer, the total amount spent *on* mining -- they are the same thing). When this value is low, 51% attacks are cheap. In 2018, BTC's security budget was [about $7 million per day](https://www.blockchain.com/charts/miners-revenue?timespan=2years&daysAverageString=7). So, the suppression of BTC (via a never-ending campaign of 51% attacks) would cost --at most-- $2.6 billion per year.
...
![image](/images/security-budget-nofees.png)
...
![image](/images/btc-fees-alltime-ma.png)
...
According to [blockchain.info](https://www.blockchain.com/charts/transaction-fees-usd?timespan=2years), fees in the last 12 months totaled $70 million. (In the 12 months before *that*, they were $770 million).

Revisit the [chart above](www.truthcoin.info/blog/security-budget#2-the-block-subsidy), and you will see that this barely registers. After all, when $70 M is priced in the units of the chart (billions), it is just $0.07.
...
Keep in mind, that the fees paid to LN-hubs[^n] will, by definition, *not* be paid to miners. So, there is no sense in which LN-fees "accumulate" into one big on-chain txn-fee (in contrast to how *the economic effect* of each LN-txn *does* accumulate into a single net on-chain txn).

[^n]: By "fees paid to LN-hubs", I mean the fees that you would pay, (off chain), to any Lightning Node that your LN-payment routes through.
...
![image](/images/fee-elasticity-table.png)
